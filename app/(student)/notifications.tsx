                ...
                >
                    {NOTIFICATIONS.map((notif) => (
                        <TouchableOpacity
                            key={notif.id}
                            activeOpacity={0.8}
                            className={`rounded-[20px] p-5 mb-4 shadow-sm border ${notif.read ? 'bg-white border-gray-100' : 'bg-[#EFF6FF] border-[#60A5FA]'}`}
                            style={!notif.read ? {
                                shadowColor: '#3B82F6',
                        ...
                        >
                            <View className="flex-row items-start justify-between">
                                <Text
                                    className={`flex-1 text-[13px] font-bold leading-5 mr-4 ${notif.read ? 'text-[#1C274C]' : 'text-[#1C274C]'}`}
                            ...
                            {notif.note && (
                                <View className="mt-3 bg-white rounded-[10px] p-3">
